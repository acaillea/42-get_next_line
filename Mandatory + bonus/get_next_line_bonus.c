...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || fd > OPEN_MAX || BUFFER_SIZE <= 0)
		return (NULL);
	rd = 1;
	while (rd > 0 && (!(ft_strchr(mem_buff[fd], '\n'))))
	{
		...
		buff[rd] = '\0';
		tmp = mem_buff[fd];
		mem_buff[fd] = ft_strjoin(mem_buff[fd], buff);
		ft_2free (buff, tmp);
	}
	line = get_return_line(mem_buff[fd]);
	mem_buff[fd] = get_next_buff(mem_buff[fd]);
	return (line);
}
